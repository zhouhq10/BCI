    ...
    for i in range(batch_size):
        for j in range(channels):
            if(eig_mode=='eig'):
                ...
                #P = th.from_numpy(np.nan_to_num(np.array(P)))
                a = th.from_numpy(np.nan_to_num(np.array(P[i,j].detach().numpy())))
                U[i,j],S[i,j],_=th.svd(a)
                #U[i,j],S[i,j],_=th.svd(P[i,j])
                #[i,j],S[i,j],_=th.svd(P[i,j]+ 1e-4*P.mean()*th.rand(l, h))
    S_fn=op.fn(S,param)
