...
class NewBatchNormSPD(nn.Module):
    """
    Input X: (N,h) SPD matrices of size (n,n) with h channels and batch size N
    Output P: (N,h) batch-normalized matrices
    SPD parameter of size (n,n)
    ...
    def forward(self,X, Y):
        Nx,hx,nx,nx=X.shape
        Ny,hy,ny,ny=Y.shape
        X_batched=X.permute(2,3,0,1).contiguous().view(nx,nx,Nx*hx,1).permute(2,3,0,1).contiguous()
        Y_batched=Y.permute(2,3,0,1).contiguous().view(ny,ny,Ny*hy,1).permute(2,3,0,1).contiguous()
        if(self.training):
            mean=functional.BaryGeom(Y_batched)
            with th.no_grad():
                self.running_mean.data=functional.geodesic(self.running_mean,mean,self.momentum)
            X_centered=functional.CongrG(X_batched,mean,'neg')
        else:
            X_centered=functional.CongrG(X_batched,self.running_mean,'neg')
        X_normalized=functional.CongrG(X_centered,self.weight,'pos')
        return X_normalized.permute(2,3,0,1).contiguous().view(nx,nx,Nx,hx).permute(2,3,0,1).contiguous()
